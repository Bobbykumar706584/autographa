...
export default function TargetLanguagePopover({ projectType }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (lang?.length < 2 || langCode?.length < 2 || lang?.length > 40 || langCode?.length > 20) {
      ...
      const result = languages.filter((l) => l?.ang?.toLowerCase() === lang?.toLowerCase() && l?.lc?.toLowerCase() === langCode?.toLowerCase() && l?.scriptDirection?.toLowerCase() === direction?.toLowerCase());
      if (result.length === 0) {
              setLanguage({
